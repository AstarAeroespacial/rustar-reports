...
\item Calcular la corrección por efecto \gls{efectodoppler} para ajustar las frecuencias de transmisión y recepción.
\end{itemize}
...
Además, se proporciona la corrección por efecto \gls{efectodoppler}, calculada en Hz, que permite ajustar las frecuencias de transmisión y recepción de la estación para compensar el desplazamiento de frecuencia durante la ventana de visibilidad.

La corrección por efecto \gls{efectodoppler} se calcula con la siguiente aproximación (convención: $\dot{\rho}>0$ si el satélite se aleja):
\[
...
En cuanto a la corrección por efecto \gls{efectodoppler}, comparamos los resultados de nuestros cálculos con una biblioteca de Python llamada \gls{skyfield}.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\linewidth]{images/validacion_doppler.png}
  \caption{Comparación de la corrección \gls{efectodoppler} calculada por el módulo y la referencia (\gls{skyfield}).}
