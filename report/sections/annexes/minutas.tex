...
\textit{Comunicación cliente-servidor:} Se evaluó el uso de colas de mensajería para comunicar el cliente con el servidor. Por ejemplo utilizando ZeroMQ o similares. Se evaluarían alternativas.
...
Un mensaje importante es la actualización del TLE a seguir, ya que esto afecta tanto al trackeo (para el módulo de control de la antena) como a la (de)modulación (por la corrección por \hyperref[gls:efectodoppler]{Doppler}).

\textit{GNU Radio:} Se intentaría hacer andar el flowgraph de GNU Radio que utiliza gr-leo para simular comunicaciones considerando \hyperref[gls:efectodoppler]{Doppler} y ruido ambiente. Se buscaba lograr generar archivos IQ realistas, para utilizar como input para todo el resto del sistema, en forma de testing.
...
\textit{Protocolo de comunicación, (de)framing, (de)modulación:} Se evaluarían librerías open source en Rust para el protocolo AX.25 o HDLC y la etapa de (de)modulación con AFSK. Se utiliza esta combinación de modulación AFSK con framing AX.25 o HDLC por ser la más común en satélites amateur.
...
\begin{itemize}
  ...
  \item Seguir investigando sobre los crates existentes de HDLC y relacionados para determinar su utilidad en el proceso de framing.
...
\begin{itemize}
  ...
  \item Implementar los otros campos de HDLC (address, control, payload).
...
\begin{itemize}
  ...
  \item Integrar ambos en un sólo módulo \texttt{hdlc}.
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item App embebida: hdlc, tracking, cli.
...
\begin{itemize}
  ...
  \item Utilizar MQTT como message broker.
\end{itemize}
...
\textit{Diagrama de la arquitectura para entender el uso de MQTT:}
\begin{itemize}
...
\begin{itemize}
  ...
  \item Resource paths.
  \item Conexión con MQTT: usando mosquitto.
  \item Conexión con BBDD: por ahora SQLite pero se migrará a PostgreSQL.
\end{itemize}

\textbf{Conclusiones:}
\begin{itemize}
  \item Utilizar un archivo de configuración para setear los tópicos MQTT de las estaciones terrenas a los que la app se tiene que suscribir al iniciarse.
  \item Utilizar un MQTT Messenger (en principio otro servicio dentro de la app).
\end{itemize}
...
\begin{itemize}
  ...
  \item Formato de los datos a transmitir por MQTT.
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Dashboard de Grafana conectado con sql o mqtt.
  \end{itemize}
  \item Informe final.
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Ver soapysdr y hacer un programa de prueba para el día que vayamos a la facultad.
  ...
  \begin{itemize}
    ...
    \item Crear archivo de configuración (toml) con coordenadas del observer, TLE y ground station id.
  \end{itemize}
...
\begin{itemize}
  ...
  \item Uso de mqtt o http para comunicación entre centro de control y app de la ground station.
  \begin{itemize}
...
\begin{itemize}
  ...
  \item Agregar en `Tracking' una feature para setear/cambiar el TLE del satélite a trackear.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Server
    \begin{itemize}
      \item Parte que lee del broker MQTT
      \item Endpoints
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Arquitectura general: agregar diagramas. Jobs/observaciones (similar a satnogs), telemetría, telecomandos.
    \item Estación terrena: descripción general (partes, responsabilidades, scheduler). Tracking, Mod/demod (GNU Radio, AFSK1200), Framing/deframing (HDLC), SDR, antenna controller.
    \item Broker MQTT: hablar de los QoS.
  ...
  \item Para la sección ``Referencias'': Introduction to satellite ground segment systems engineering, SatNOGS, etc.
