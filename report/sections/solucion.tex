...
La interfaz de usuario del sistema proporciona una plataforma web moderna e intuitiva para la gestión y monitoreo de satélites y estaciones terrenas. Desarrollada con tecnologías web modernas (Next.js, React y TypeScript), la interfaz ofrece una experiencia fluida, responsiva y accesible desde cualquier navegador web, adaptándose automáticamente a diferentes dispositivos desde teléfonos móviles hasta monitores de escritorio.

La aplicación se estructura en dos módulos principales: Satélites y Estaciones Terrenas. El usuario puede alternar entre estos módulos mediante una barra de navegación superior. La interfaz se comunica con la API del sistema backend para obtener y enviar datos en tiempo real, garantizando que la información presentada esté siempre actualizada.
...
El usuario puede visualizar en tiempo real la posición de cualquier satélite sobre un mapa interactivo de la Tierra. El sistema calcula y actualiza continuamente la trayectoria orbital, mostrando la ubicación actual del satélite segundo a segundo. Junto al mapa, se presenta información precisa en tiempo real: coordenadas geográficas exactas (latitud y longitud) y altitud sobre el nivel del mar en kilómetros.

Una funcionalidad importante es la capacidad de actualizar los parámetros orbitales del satélite. Cuando se obtienen datos TLE (elementos orbitales) más recientes y precisos, el usuario puede ingresarlos directamente en el sistema, mejorando inmediatamente la exactitud del seguimiento orbital.
