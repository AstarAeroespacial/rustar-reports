...
Los detalles de la implementación del protocolo de enlace utilizado para los ensayos de este trabajo profesional se encuentran en Anexo~\ref{sec:hdlc}.
...
\begin{itemize}
    ...
    \item La corrección de frecuencia por efecto \gls{efectodoppler}, tanto para la transmisión (\textit{uplink}) como para la recepción (\textit{downlink}), compensando el desplazamiento de frecuencia causado por el movimiento relativo entre la estación y el satélite.
\end{itemize}

Estos valores se actualizan periódicamente durante toda la ventana de observación. Los ángulos calculados se envían al controlador de antena para ejecutar el seguimiento físico del satélite, mientras que las correcciones de frecuencia se aplican al dispositivo \gls{SDR} para mantener sintonizadas las frecuencias de comunicación a pesar del efecto \gls{efectodoppler}.
...
El módulo de controlador de antena constituye la interfaz entre el sistema de software y el hardware físico responsable de la orientación de la antena. Su función es traducir los ángulos de acimut y elevación, calculados por el módulo de seguimiento, en comandos que el controlador físico de la antena pueda ejecutar.

Durante una ventana de observación, el módulo de seguimiento calcula continuamente la posición del satélite y determina los ángulos necesarios para mantener la antena apuntando hacia él. Estos valores se envían al controlador de antena para su ejecución.

El controlador formatea esta información según el protocolo específico del hardware de la antena y se la envía. El hardware de la antena procesa estos comandos y ajusta la orientación física de la antena en tiempo real, permitiendo mantener el enlace de comunicación durante todo el pase del satélite.

Esta arquitectura modular permite separar la lógica de cálculo de posición orbital de los detalles específicos del hardware de control de antena, facilitando la integración con diferentes sistemas de orientación sin modificar el resto de los componentes del sistema.

Los detalles se describen en el Anexo~\ref{sec:antenna_controller}.
