...
La capa de abstracción para interactuar con el dispositivo SDR se implementa utilizando SoapySDR. A través de su interfaz, se puede inicializar y configurar una gran variedad de dispositivos SDR, para luego enviar y recibir muestras de la señal digitalizada.
...
La integración con GNU Radio se realiza ejecutando los flowgraphs definidos en Python como procesos externos. La aplicación inicializa y controla la ejecución de estos procesos, permitiendo elegir de forma dinámica el flujo a utilizar en función del esquema de modulación implementado por la misión. La comunicación entre la aplicación y el flowgraph se realiza mediante ZeroMQ.

\subsubsection{Protocolo de enlace}

\subsubsection{Seguimiento}

El módulo de Tracking es fundamental para el funcionamiento de la estación terrena, ya que permite seguir la trayectoria de los satélites durante las ventanas de observación. Su propósito principal es calcular, en tiempo real, los parámetros necesarios para mantener el enlace con el satélite a medida que este se desplaza en su órbita.

Durante la ejecución de una tarea de observación, el módulo de Tracking utiliza los parámetros orbitales del satélite (proporcionados en la tarea mediante el TLE) para determinar continuamente:

\begin{itemize}
    \item Los ángulos de azimut y elevación necesarios para orientar la antena y mantenerla apuntando al satélite.
    \item La corrección de frecuencia por efecto Doppler, tanto para la transmisión (uplink) como para la recepción (downlink), compensando el desplazamiento de frecuencia causado por el movimiento relativo entre la estación y el satélite.
    \item La posición y velocidad del satélite en cada instante, permitiendo predecir su trayectoria durante el pase.
\end{itemize}

Estos valores se actualizan periódicamente durante toda la ventana de observación. Los ángulos calculados se envían al controlador de antena para ejecutar el seguimiento físico del satélite, mientras que las correcciones de frecuencia se aplican al dispositivo SDR para mantener sintonizadas las frecuencias de comunicación a pesar del efecto Doppler.

La implementación del módulo de Tracking se describe en detalle en el Anexo~\ref{sec:tracking}.
